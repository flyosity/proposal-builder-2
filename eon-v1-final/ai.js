...
$(function() {
    ...
    /**
     * Forced focus class on LI for input field styling
     */
    /*
    $(document).on('focus', 'ul.fields li:not(.focus) input[type="text"], ul.fields li:not(.focus) input[type="password"]', function() {
        $(this).parents('li').addClass('focus');
    });

    $(document).on('input', 'ul.fields li.focus input[type="text"], ul.fields li.focus input[type="password"]', function() {
        if ($(this).val() == '') {
            $(this).parents('li').removeClass('focus');
        }
    });
    */
    ...
    $(document).on('click', '.top-right-buttons .get-started', function() {
        ...
        /*
        $('.exp-pubilc-web').fadeOut( 250, "linear", function() {
            $('.account-create').fadeIn( 250, "linear", functiion(){} );
        });
        */
        return false;
    });
